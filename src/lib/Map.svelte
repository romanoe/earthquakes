<script>
  ...
  // Earthquakes API url
  const url =
    "https://emidius.mi.ingv.it/fdsnws/event/1/query?starttime=2000-01-01T00:00:00&endtime=2022-12-31T23:59:59&orderby=time-asc&limit=4000&format=json";
  ...
  const initZoom = () => {
    select("svg").call(mapZoom).on("wheel.zoom", null);
  };
    ...
    select("svg")
      ...
      .call(
        ...
        zoomIdentity
          .translate(w / 2, h / 2)
          .scale(scale)
          .translate(-projection(coordinates)[0], -projection(coordinates)[1])
      );
  };

  // Fetch data and initialize zoom
  onMount(async () => {
    temporalEarthquakes = await fetch(url)
      .then((response) => response.json())
      .then((json) => json.features);
    const sortedEarthquakes = temporalEarthquakes
      .slice()
      .sort((a, b) => ascending(a.properties.mag, b.properties.mag));
    cities = await fetch("./data/cities.geojson")
      .then((response) => response.json())
      .then((json) => json.features);
    biggestEarthquakes = sortedEarthquakes.slice(-10);

    borders = await fetch("./data/limits_IT_regions.geojson")
      .then((response) => response.json())
      .then((json) => json.features);
...
<main>
  {#if !mobile}
      ...
      <Row>
        <Column>
          <div id="map">
            <svg width={w} height={h}>
              ...
              <g fill="white" stroke="grey" stroke-width="0.1">
                {#each borders as border, i}
                  <path
                    d={path(border)}
                    in:draw={{ delay: i * 50, duration: 1500 }}
                  />
                {/each}
              </g>
                ...
                {#each temporalEarthquakes as earthquake, i}
                  <circle
                    ...
                    on:mouseover={showTooltip(
                      event,
                      earthquake.properties.source_id
                    )}
                ...
                {#each biggestEarthquakes as earthquake, i}
                  ...
                  >
                    <animate
                      attributeName="r"
                      from="0"
                      to={Math.pow(1.3, earthquake.properties.mag)}
                      dur="3s"
                      begin="0s"
            ...
            {#each temporalEarthquakes as earthquake, i}
              <div
                id={earthquake.properties.source_id}
                display="none"
    ...
    <footer>
      <small
        ><code>Dati: Istituto Nazionale Geofisica et Vulcanologia</code></small
      >
    </footer>
  {/if}
</main>
